//! Usage:
//!   cargo run --example token_transfers -- --bind 0.0.0.0:8001
...
async fn main() -> anyhow::Result<()> {
    ...
    pipeline = pipeline
        ...
        .with_kind_filter(vec![
            InstructionKind::Transfer,
            InstructionKind::TransferChecked,
        ])
        .on_instruction(Box::new(|inst| {
            println!(
                "TRANSFER {:?} [slot {}] amount={:?} from={} | sig={}",
                inst.kind, inst.slot, inst.input_amount, inst.authority, inst.signature,
            );
        }));

    tracing::info!(bind = %args.bind, "Streaming SPL token transfers...");
    pipeline.run().await
}
